				...
				$('form').submit(function(e) {
					...
					if (uploader.files.length > 0) {
						...
						uploader.bind('FileUploaded', function (up, file, info) {
							...
							var file_original = '<input type="hidden" name="finished_names[]" value="'+obj.FileName+'" />';
							var file_file = '<input type="hidden" name="file['+n+'][file]" value="'+obj.NewFileName+'" />';
							var file_name = '<input type="hidden" name="file['+n+'][name]" value="'+obj.FileName+'" />';
							//var file_latitude = '<input type="hidden" name="file['+n+'][latitude]" value="'+obj.latitude+'" />';
							//var file_longitude = '<input type="hidden" name="file['+n+'][longitude]" value="'+obj.longitude+'" />';
                                                        //var file_longitude = '<input type="hidden" name="file['+n+'][lastModified]" value="'+file.lastModified+'" />';
							$('form').append(new_file_field);
							$('form').append(file_original);
							$('form').append(file_file);
							$('form').append(file_name);
							//$('form').append(file_latitude);
							//$('form').append(file_longitude);
							//alert("file="+obj.NewFileName +"/"+obj.FileName);
							n++;
