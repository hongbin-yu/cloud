...
class process {
	...
	private function signup() {
		...
		$add_client_data_user  = $_POST['username'];
        $add_client_data_name  = $_POST['username'];	
		$new_arguments = array(
								'id'		=> '',
	...
	private function signinOrSignup() {
		...
		if ($this->count_user > 0){
			...
			if (strpos($this->sysuser_username,"@")>0) {
			    signup();
			} else { 	
				$this->errorstate = 'invalid_credentials';
			}
	...
	private function login() {
		...
		if(!empty($_GET['password'])) {
			header('Location: '. BASE_URI. '?error='.$this->login_err_message);
		}
		//echo $this->login_err_message;
	...
	private function download_file($action) {
		...
		if (isset($_GET['id'])) {
			...
			if (isset($this->check_level) && in_session_or_cookies($this->check_level)) {
					 ...
					 * Get the file name
					 */
					$this->statement = $this->dbh->prepare("SELECT url, original_url,filename, expires, expiry_date FROM " . TABLE_FILES . " WHERE id=:id");
					...
					if ($this->can_download == true) {
						...
						$pathinfo =  pathinfo($this->real_file);
						if (file_exists($this->real_file)) {
							...
							if($action==0) {
								header('Content-Disposition: attachment; filename="'.$this->filename.'.'.$pathinfo['extension'].'"');
							}
							header('Expires: 0');
