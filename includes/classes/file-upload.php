...
class PSend_Upload_File
{
	...
	function upload_move($arguments)
	{
		...
		$this->safe_filename = $this->safe_rename($this->filename);
		$this->filename_on_disk = time().'-'.$this->makehash.'-'.$this->safe_filename;
	...
	function upload_add_to_database($arguments)
	{
		...
		$this->description		= encode_html($arguments['description']);
                ...
                $this->lastModified             = (!empty($arguments['lastmodified'])) ? date("Y-m-d", $arguments['lastmodified']) : date("Y-m-d");
		...
		if (isset($arguments['add_to_db'])) {
			$this->statement = $this->dbh->prepare("INSERT INTO " . TABLE_FILES . " (url, original_url, filename,filepath,lastmodified, description, uploader, expires, expiry_date, public_allow, public_token)"
											."VALUES (:url, :original_url, :name,:filepath, :lastmodified, :description, :uploader, :expires, :expiry_date, :public, :token)");
			...
			$this->statement->bindParam(':name', $this->name);
                        $this->statement->bindParam(':filepath', $this->filepath);
                        $this->statement->bindParam(':lastmodified', $this->lastModified);
